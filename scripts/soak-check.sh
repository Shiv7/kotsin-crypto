...
print(f"channels: {{{', '.join(f'{k}:{v[\"count\"]}' for k, v in f['channels'].items())}}}" + (f"  STALE: {stale}" if stale else ""))
...
print(f"signals: {c.get('signals', 0)} · decisions {sig} · exits {{{', '.join(f'{k[5:]}:{v}' for k, v in c.items() if k.startswith('exit_'))}}} · ratchets {c.get('ratchets', 0)} · funding events {c.get('funding_events', 0)}")
